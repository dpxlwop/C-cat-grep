...
int main(int argc, char **argv) {
  ...
  while (
      (rez = getopt_long(argc, argv, "bensta", long_options, &option_index)) !=
  ...
  for (int i = optind; i < argc; i++) {
    if (file_proccess(argv[i], flag_list, flag_counter) != 0)
      return 1;
  }
...
int file_proccess(const char *filename, int *flags, int flag_count) {
  ...
  // переменные флагов
  for (int i = 0; i < flag_count;
       i++) { // переносим флаги из списка в переменные
    switch (flags[i]) {
    case 1:
      flag_b = 1;
      break;
