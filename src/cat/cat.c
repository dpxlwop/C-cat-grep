...
int main(int argc, char** argv) {
    ...
    while ((rez = getopt_long(argc, argv, "benstaET", long_options, &option_index)) !=
        ...
        if (rez == 'b')  // нумеровать непустые строки
            flag_list[flag_counter++] = 1;
        else if (rez == 'e' || rez == 'E')  // отображать $ в конце каждой строки
            flag_list[flag_counter++] = 2;
        else if (rez == 'n')  // нумеровать все строки
            flag_list[flag_counter++] = 3;
        else if (rez == 's')  // сжимать несколько пустых строк в одну
            flag_list[flag_counter++] = 4;
        else if (rez == 't' || rez == 'T')  // отображать \t как ^I
    ...
    for (int i = optind; i < argc; i++)
        (file_proccess(argv[i], flag_list, flag_counter) != 0)

            int
            normalize_flags(int* flags, int count) {
