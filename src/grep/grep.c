...
void file_proccess(int argc, char** argv, flags flag_container, char** e_args,
  ...
  for (int i = file_index; i < argc; i++) {
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
      ...
      if (flag_container.e) {  // если есть -e
        for (int k = 0; k < e_args_counter; k++) {
          if ((match = search_in_line(line, e_args[k], flag_container.i,
                                      result)) == 1) {
            k = e_args_counter;
          }
        }
      ...
      if (match) {
        count_matching_str++;
        if (!flag_container.c &&
            !flag_container.l) {  // если совпадение, то вывод по флагам
          if (argc - file_index == 1)  // если один файл
    ...
    if (flag_container.c && flag_container.l && count_matching_str >= 1)
      count_matching_str =
          1;  // если есть -c и -l и есть совпадения, то выводим 1 (GNU правила)
    if (flag_container.c &&
        argc - file_index ==
            1)  // вывод чисто количества строк, без имени файла, если один файл
      printf("%d\n", count_matching_str);
...
int search_in_line(const char* line, const char* pattern, int ignore_case,
  ...
  if (ignore_case) {
    match =
        (strcasestr(line, pattern) != NULL);  // ищем строку без учета регистра
  } else {
